<script lang="ts">
  ...
  selectedRowElements = rowElements.map((
    { title, has_high_priority, categorie, created_at, last_assignment_date, name_by_domain }: any) => ({ title, has_high_priority, categorie, last_assignment_date, created_at, name_by_domain }
  ));
  selectedValues = selectedRowElements

  if (domain === 'traducteur') {
    selectedRowElements = rowElements.map((
      { title, has_high_priority, categorie, language, created_at, last_assignment_date, name_by_domain }: any) => ({ title, has_high_priority, categorie, language, created_at, last_assignment_date, name_by_domain }
    ));
    selectedValues = selectedRowElements
  }
  if (domain === 'post-prod') {
    selectedRowElements = rowElements.map((
      { title, has_high_priority, categorie, created_at, last_assignment_date, name_by_domain, version }: any) => ({ title, has_high_priority, categorie, created_at, last_assignment_date, name_by_domain, version }
    ));
    selectedValues = selectedRowElements
  }
</script>
